...
# Always set this â€” pipeline needs non-zero exit on error
...
**Rules:**
- `[CmdletBinding()]` + `param()` block â€” all values come from parameters, never from the environment directly
- `$ErrorActionPreference = 'Stop'` â€” ensures any cmdlet error bubbles up
- `try/catch` wrapping main logic â€” always `exit 1` on failure
- `Write-Host` for progress (captured in ADO pipeline log)
- `Write-Error` before `exit 1` â€” shows red error in pipeline log
- **No `Write-Host` of secrets** â€” pass secrets as pipeline variables marked as `secret`
...
### Purpose
ALM Toolkit (free, https://alm-toolkit.com) is used for schema-only SSAS Tabular model comparisons and deployments. Unlike Tabular Editor deploy (which always does a full metadata replace), ALM Toolkit identifies delta changes and can deploy only what changed â€” reducing processing requirements and deployment risk.
...
| Promoting a model from DEV â†’ TEST with structural changes | ALM Toolkit |
...
1. **File vs Server**: compare a `.bim` file (artifact) to a live SSAS instance â€” used in release pipelines
2. **Server vs Server**: compare DEV SSAS to TEST SSAS directly â€” used for drift detection
3. **File vs File**: compare two `.bim` artifacts â€” used for code review / diff
...
Note: `$(sass_server)` is the intentional historical variable name â€” do not correct to `ssas_server`.

### Drift detection (Server vs Server)
Run this check as part of the DEV â†’ TEST promotion gate to detect configuration drift:
...
- **Partitions** â€” never overwrite; partition management is handled separately
- **Role memberships** â€” AD group assignments are environment-specific; overwriting deletes them
- **Data sources** â€” connection strings are environment-specific

In ALM Toolkit GUI: Options â†’ Deployment â†’ check "Retain Partitions", "Retain Role Members", "Retain Data Sources".
...
### Review checklist (Mode G â€” Deployment Review)
| Check | Severity if failed |
|---|---|
| ALM Toolkit used for delta deployments (not always full replace) | ðŸŸ¡ MEDIUM |
| Partitions preserved during ALM Toolkit deployment | ðŸ”´ CRITICAL |
