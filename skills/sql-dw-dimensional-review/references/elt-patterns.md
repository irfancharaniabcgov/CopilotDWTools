...
> Optional BIML guide (if using BIML for package generation): https://www.kingswaysoft.com/blog/2018/12/21/Using-BIML-with-KingswaySoft-SSIS-Components
...
> **BIML Express is not widely maintained.** Manual package development in Visual Studio
> SSIS Designer is the primary approach. Use BIML only if your team is already familiar
> with it and has a licensed copy of BIML Studio, or if you have a large number of
> identical packages that justify the setup cost.
>
> The reference project `SNOW_DW` uses BIML for flat-file → staging package generation
> as an example of the technique. See `C:\Projects\SNOW_DW\SSIS\SNOW_DW_SSIS\*.biml`.

When loading **many** Salesforce objects with the same pattern (incremental source → staging),
BIML + BIMLScript can generate packages instead of building each one manually.

**Tools (choose one):**
- **BIML Studio** — commercial IDE; full BIML features including import/export. Recommended if committing to BIML.
- **BIML Express** — free Visual Studio add-in; limited support, development has slowed.
  Sufficient for simple generation tasks but not recommended for new projects.
- **BIML Online** — web-based IDE; useful for reverse-engineering existing packages to BIML.

**Recommended workflow (if using BIML):**
1. Build **one** Salesforce → Staging package manually in Visual Studio using the KingswaySoft designer
2. Reverse-engineer it to BIML (BIML Studio: **Import SSIS Packages**; BIML Express: right-click → **Convert SSIS Packages to BIML**)
...
5. Generate `.dtsx` files from the `.biml` source; commit the **generated `.dtsx` files** to git — not just the BIML source

> **Key tip**: The reverse-engineer step is the most reliable way to get the exact XML
...
These issues cause package generation to fail silently or at runtime:
...
Applies to all .NET primitives: `System.String` → `String`, `System.Double` → `Double`,
`System.Decimal` → `Decimal`, `System.Boolean` → `Boolean`, `System.DateTime` → `DateTime`.
...
> The `ComponentClassID` GUIDs are captured when you reverse-engineer an existing package.
> Always copy from the reverse-engineered output rather than looking them up manually.
...
> **Reference project example — flat file BIML (SNOW_DW):**
> `SNOW_DW` uses BIML to generate flat-file → staging packages following the same numbered-file
> convention (`01-Connections.biml`, `02-FlatFileFormat.biml`, `30-LoadStaging.biml`, etc.).
> This is a good working example of BIML in this stack — review those files if adopting BIML
> for CSV-based source systems.
