...
For every undocumented object, the agent should produce a **draft description** before asking the user. The user should be reviewing concrete text, not answering blind. Heuristics by object type:
...
### 2.2 Column name patterns

| Pattern | Likely meaning | Draft description |
|---|---|---|
| `*ID`, `*Key`, `*Code` | Identifier or FK | "Identifier for [entity]." If suffixed `Key` and integer → "Surrogate key." If suffixed `ID` → "Natural identifier from source." |
| `*Date`, `*DateTime`, `*Timestamp`, `*_DT` | Date column | "Date [meaning] occurred." For DW: also flag for Calendar FK conversion. |
| `Is*`, `Has*`, `*Flag`, `*_YN` | Boolean | "Indicator: [Yes value] / [No value]." Cardinality check (Q7) confirms domain. |
...
| `*Address*`, `*City*`, `*PostalCode*`, `*Phone*`, `*Email*` | PII contact | "Contact information. Classification: Contact Info." |
...
| `RowVersion`, `Timestamp` (binary) | Concurrency token | "Row version concurrency token. Not for reporting." |
| `Created*`, `Modified*`, `Updated*` | Audit timestamps | "Audit timestamp — [created / last modified] in source system." |
| Sentinel values `-1`, `'1753-01-01'`, `'9999-12-31'` in any column | DW sentinel | "Sentinel: [unknown / open-ended]." |
...
The `db-documenter` agent processes undocumented objects in **batches grouped by table**, not one at a time:
...
3. For each table in the worklist:
   - Run the appropriate column audit (Q-SRC-2 / Q-DW-2 / Q-SSAS-2) restricted to that table
   - Generate drafts for table + all its undocumented columns using the inference heuristics (§ 2)
   - Present the batch to the user in one message — table description first, columns in a markdown table second
   - User reviews, confirms, or revises in one round
   - Write back per the target's write mechanism (§ Authority & Scope)
4. After all tables in scope: re-run the audit to confirm 100% coverage; report any objects skipped with reason
...
| Object type | Total | Documented before | Documented after | Coverage % |
|---|---|---|---|---|
| Tables | N | N | N | XX% |
| Views | N | N | N | XX% |
| Columns | N | N | N | XX% |
| Stored procedures | N | N | N | XX% |
| SSAS measures | N | N | N | XX% |
